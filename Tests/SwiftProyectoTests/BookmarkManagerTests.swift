...
final class BookmarkManagerTests: XCTestCase {
  ...
  func testWithAccessListDirectory() throws {
    ...
    try FileManager.default.createDirectory(at: testDir, withIntermediateDirectories: true)
    try "file1".write(
      to: testDir.appendingPathComponent("file1.txt"), atomically: true, encoding: .utf8)
    try "file2".write(
      to: testDir.appendingPathComponent("file2.txt"), atomically: true, encoding: .utf8)
    let bookmarkData = try BookmarkManager.createBookmark(for: testDir)
  ...
  func testNestedBookmarkAccess() throws {
    ...
    let parentExists = try BookmarkManager.withAccess(parentDir, bookmarkData: parentBookmark) {
      url in
