...
final class ModelContainerFactoryTests: XCTestCase {
  ...
  func testContainerErrorDescriptions() {
    ...
    let projectRootError = ModelContainerFactory.ContainerError.projectRootDoesNotExist(
      tempDirectory)
    XCTAssertNotNil(projectRootError.errorDescription)
    XCTAssertTrue(projectRootError.errorDescription!.contains("Project root"))

    let cacheDirectoryError = ModelContainerFactory.ContainerError.cacheDirectoryCreationFailed(
      tempDirectory, testError)
    XCTAssertNotNil(cacheDirectoryError.errorDescription)
    XCTAssertTrue(cacheDirectoryError.errorDescription!.contains("cache"))

    let containerCreationError = ModelContainerFactory.ContainerError.containerCreationFailed(
      testError)
