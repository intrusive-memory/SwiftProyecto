...
final class ModelManagerBareDescriptorTests: XCTestCase {
  func testBareDescriptorIsRegistered() async throws {
    ...
    XCTAssertEqual(
      component?.id, LanguageModel.id, "Component ID should match LanguageModel constant")
    ...
    XCTAssertGreaterThan(
      component?.minimumMemoryBytes ?? 0, 0, "Component should have minimumMemoryBytes")
